...
def main():
    ...
    bounceend2=0    # end of bounce for first player in bouncelist, or zero if list is empty
        ...
        with Usbserial() as myusb:
            while True:
                ...
                if c: # data from controller 
                    ...
                    # new bounceend2 with bouncelist
                    if bounceend2 > 0 and timenow > bounceend2:
                        bounceend2=0 # should just set at end based on array
                        for i in bouncelist:
                            player=players[i]
                            oldstate=player['sit']
                            if player['sitnew'] != oldstate:    # player in bounce mode and not same as first
                                ...
                                if bounceend2:
                                    bounceend2 = min(bounceend2,player['lastchg'] + bouncetime)
                                else: # bounceend2 is zero
                                    bounceend2 = player['lastchg'] + bouncetime
                    # temp, chk bounceend at intervals even if bouncelist is empty
                    if timenow > bounceend:
                        bounceend=0
                        for i in range(max):
                            player=players[i]
                            oldstate=player['sit']
